...
// 日時表示コンポーネント（通常時はコンパクト表示、変更時は見やすい特大モーダル）
function BigDateDisplay({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <>
      {/* 通常表示エリア（控えめで邪魔にならないレイアウト） */}
      <div className="bg-slate-50 border-2 border-slate-300 rounded-2xl p-3 flex justify-between items-center gap-2">
        ...
        <button
          type="button"
          onClick={handleOpen}
          className="px-3 py-2 bg-white hover:bg-slate-100 active:bg-slate-200 border-2 border-slate-300 rounded-xl text-xs font-black text-blue-700 shadow-sm flex items-center gap-1 shrink-0"
        >
          <span>🕒</span>
          <span>変更</span>
        </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border-4 border-blue-600 space-y-5">
            <div className="flex justify-between items-center border-b pb-2">
              <div className="flex items-center gap-2">
                <span className="text-2xl">🕒</span>
                <h3 className="text-lg font-black text-slate-900">記録日時の変更</h3>
              </div>
              <button
                type="button"
                onClick={() => setIsOpen(false)}
                className="text-slate-400 hover:text-slate-600 text-2xl font-bold w-10 h-10 flex items-center justify-center"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              {/* 年配の方でもタップ・操作しやすい特大入力枠 */}
              <input
                type="datetime-local"
                value={tempValue}
                onChange={(e) => setTempValue(e.target.value)}
                className="w-full h-16 px-4 border-3 border-slate-400 rounded-2xl text-xl font-black bg-slate-50 text-slate-900 focus:bg-white focus:border-blue-600 outline-none text-center shadow-inner"
              />

              <p className="text-xs text-slate-500 font-bold text-center">
                ※枠をタップすると大きな時計・カレンダーが出ます
              </p>
            </div>
            ...
            <div className="space-y-2 pt-2">
              <button
                type="button"
                onClick={handleApply}
                className="w-full h-14 bg-blue-600 hover:bg-blue-700 text-white font-black text-lg rounded-2xl shadow-lg transition-all"
              >
                この日時で決定する
              </button>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="h-12 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border-2 border-emerald-300 font-bold rounded-xl text-xs"
                >
                  ⚡ 現在時刻に戻す
                </button>
                <button
                  type="button"
                  onClick={() => setIsOpen(false)}
                  className="h-12 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-xs"
                >
                  キャンセル
                </button>
              </div>
...
function TempInputRow({
  ...
  return (
    <div className="border-t-3 border-slate-200 pt-5">
      ...
      {!valid && (
        <p className="text-xs text-red-600 font-black mt-1 px-1">
  ...
  return (
    <main className="min-h-screen bg-slate-100 text-slate-900 pb-32 font-sans">
      ...
      <div className="max-w-xl mx-auto p-4 space-y-6">
        {successMsg && (
          <div className="p-6 bg-emerald-100 border-4 border-emerald-500 text-emerald-950 rounded-2xl text-xl text-center font-black shadow-lg animate-bounce">
            ✅ {successMsg}
          </div>
        )}
        ...
        {dialogError && (
          <div className="p-5 bg-red-100 border-4 border-red-500 text-red-950 rounded-2xl shadow-xl flex flex-col items-center gap-3">
            ...
            <p className="text-base font-bold text-center leading-relaxed">{dialogError}</p>
            <button
              type="button"
              onClick={() => setDialogError('')}
              className="mt-1 px-8 py-2.5 bg-red-600 hover:bg-red-700 text-white font-black rounded-xl text-sm shadow"
            >
              閉じる
            </button>
          </div>
